...
def refresh_schedule():
    ...
    for q in range(5):  # no. of 2nd round questions per faction
        c = c + timedelta(seconds=15)  # time per 2nd round questions
        show_r2_question.apply_async(eta=c)
    c = c + timedelta(seconds=1)
    calculate_r2_scores.apply_async(eta=c)
    c = c + timedelta(seconds=60)
...
def r2_results(email):
	player = Player.query.filter_by(email=email).first()
	winner = Player.query.order_by(Player.r2_score.desc()).first()
